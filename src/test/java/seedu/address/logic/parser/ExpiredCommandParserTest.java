...
public class ExpiredCommandParserTest {
    ...
    public void parse_validDate_success() {
        ...
        assertParseSuccess(parser, " " + PREFIX_DATE_BEFORE + "2026-04-01", expectedCommand);
        assertParseSuccess(parser, " " + PREFIX_DATE_BEFORE + "2026-04-01   ", expectedCommand);
        ...
        assertParseSuccess(parser, " " + PREFIX_DATE_BEFORE + "2024-02-29", expectedCommand);
    }
    ...
    public void parse_preambleExistsBeforePrefix_failure() {
        assertParseFailure(parser, " garbage " + PREFIX_DATE_BEFORE + "2026-04-01",
                String.format(MESSAGE_INVALID_COMMAND_FORMAT, ExpiredCommand.MESSAGE_USAGE));
    }
    ...
    public void parse_duplicatePrefix_failure() {
        assertParseFailure(parser, " "
                        ...
                        + PREFIX_DATE_BEFORE + "2026-07-11",
                MESSAGE_DUPLICATE_FIELDS + PREFIX_DATE_BEFORE);
    }

    @Test
    public void parse_invalidDate_failure() {
        // No date provided
        assertParseFailure(parser, " " + PREFIX_DATE_BEFORE, ParserUtil.MESSAGE_INVALID_DATE);

        // Not a date
        assertParseFailure(parser, " " + PREFIX_DATE_BEFORE + "invalid-date", ParserUtil.MESSAGE_INVALID_DATE);

        // Invalid date format - wrong separator
        assertParseFailure(parser, " " + PREFIX_DATE_BEFORE + "01/04/2026", ParserUtil.MESSAGE_INVALID_DATE);

        // Invalid date - 31 April does not exist
        assertParseFailure(parser, " " + PREFIX_DATE_BEFORE + "2026-04-31", ParserUtil.MESSAGE_INVALID_DATE);

        // Invalid date - a year does not have 13 months
        assertParseFailure(parser, " " + PREFIX_DATE_BEFORE + "2026-13-01", ParserUtil.MESSAGE_INVALID_DATE);

        // Invalid date - 29 February does not exist on non-leap years
        assertParseFailure(parser, " " + PREFIX_DATE_BEFORE + "2026-02-29", ParserUtil.MESSAGE_INVALID_DATE);
    }

    @Test
    public void parse_extraDate_failure() {
        assertParseFailure(parser, " " + PREFIX_DATE_BEFORE + "2026-02-29 2026-02-29"
